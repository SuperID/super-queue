...
utils.getQueueKey = function (prefix, name) {
  return prefix + 'Q:' + name;
};

utils.getProcessingQueueKey = function (prefix, queue, name) {
  return prefix + 'PQ:' + queue + ':' + name;
};

utils.getCallbackKey = function (prefix, name) {
  return prefix + 'CB:' + name;
};

utils.getHeartbeatKey = function (prefix, type, name) {
  return prefix + 'H:' + type.slice(0, 1).toUpperCase() + ':' + name;
};
